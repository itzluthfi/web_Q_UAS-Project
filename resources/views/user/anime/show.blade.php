        ...
        <div class="max-w-4xl mx-auto bg-gray-800 shadow-xl rounded-lg overflow-hidden glow-effect">
            ...
            <div class="md:flex p-6">
                <div class="md:w-64 flex-shrink-0 mb-6 md:mb-0">
                    ...
                    <img class="w-full h-auto rounded-lg shadow-lg border border-gray-700" src="{{ $anime->image_url ?? ($anime['images']['webp']['image_url'] ?? asset('assets/image-notFound.jpg')) }}" alt="{{ $anime->title ?? 'Untitled' }}">
            ...
            <div class="border-t border-gray-700 mt-4 px-6 py-8">
                ...
                <form id="form-komentar" method="POST" action="{{ route('comments.store') }}" class="mb-8">
                    ...
                    <textarea id="comment-input" name="content" rows="3" placeholder="Tulis komentar kamu..." class="w-full input-dark rounded-lg p-4 mb-3 focus:outline-none focus:ring-2 focus:ring-purple-500 resize-none"></textarea>
                    <button type="submit" class="bg-purple-700 text-white px-5 py-2 rounded-lg hover:bg-purple-600 transition flex items-center btn-glow">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
                ...
                <div id="comments-section" class="space-y-4">
                    @forelse ($comments as $comment)
                        <div class="p-4 bg-gray-700/50 border border-gray-600 rounded-lg shadow-md comment-item comment-item" id="comment-{{ $comment->id }}">
                            ...
                            <div class="mt-2 flex space-x-4">
                                <!-- Tombol Balas -->
                                <button type="button" class="reply-toggle text-purple-400 hover:text-white" data-comment-id="{{ $comment->id }}">
                                    Balas
                                </button>

                                <!-- Tombol Toggle Balasan -->
                                @if (count($comment->replies ?? []) > 0)
                                    <button type="button" class="flex items-center text-blue-400 hover:text-blue-300 toggle-replies" data-comment-id="{{ $comment->id }}">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-4 h-4 mr-1 reply-icon-show">
                            ...
                            <div id="reply-form-{{ $comment->id }}" class="reply-form mt-3 p-4 bg-gray-800 rounded-md hidden">
                                <form method="POST" action="#">
                                    ...
                                    <textarea name="body" rows="2" placeholder="Tulis balasan..." class="w-full input-dark rounded-md p-2 mb-2"></textarea>
                                    <div class="flex justify-end space-x-2">
                                        <button type="button" onclick="closeReplyForm({{ $comment->id }})" class="px-3 py-1 text-sm bg-gray-700 hover:bg-gray-600 rounded">Batal</button>
                                        <button type="submit" class="px-3 py-1 text-sm bg-purple-700 hover:bg-purple-600 rounded btn-glow">Kirim</button>
                                    </div>
                                </form>
                            </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    @forelse($relatedAnimes as $relatedAnime)
                        <div class="bg-gray-800 rounded-lg overflow-hidden border border-gray-700 hover:border-purple-500 transition related-anime-card">
                            ...
                            <img src="{{ $relatedAnime->image_url ?? ($relatedAnime->image_url ?? 'https://via.placeholder.com/400x600?text=No+Image ') }}" alt="{{ $anime['title'] ?? 'Anime Title' }}" class="w-full h-48 object-cover">
                            <div class="p-2">
